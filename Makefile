...
all: build $(BIN_DIR)/otguard-web $(BIN_DIR)/otguardd $(BIN_DIR)/otguard-mksecret $(BIN_DIR)/otguard-purgerules $(SHARE_DIR)/login.html $(ETC_DIR)/cert.pem $(ETC_DIR)/key.pem $(ETC_DIR)/secrets $(SYSTEMD_DIR)/otguardd.service $(CRON_DIR)/otguard-cron
...
dist:
	tar -czvf otguard-$(VERSION).tgz build/ README.md LICENSE.md install
